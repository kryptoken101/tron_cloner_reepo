...
WEBHOOK_URL = os.getenv("TRON_TX_WEBHOOK")  # Set your Slack/Discord webhook URL here

# Load mainnet private key securely
def load_mainnet_private_key():
    encoded = os.getenv("TRON_MAINNET_KEY")
    if encoded:
        try:
...
def log(msg: str):
    ...
    if WEBHOOK_URL:
        try:
            requests.post(WEBHOOK_URL, json={"content": log_line})
        except Exception:
            pass
...
def main():
    ...
    if args.batch:
        ...
        for idx, txid in enumerate(txids):
            log(f"Processing {idx+1}/{len(txids)}: {txid}")
